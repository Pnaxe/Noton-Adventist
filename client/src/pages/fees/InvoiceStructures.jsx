...
const InvoiceStructures = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validItems.length === 0) {
      const errorMsg = 'Please add at least one invoice item';
    ...
    try {
      if (selectedStructure) {
        ...
        showToast('Invoice structure updated successfully', 'success');
      } else {
        await axios.post(`${BASE_URL}/fees/invoice-structures`, submitData, {
          headers: { Authorization: `Bearer ${token}` }
        });
        showToast('Invoice structure created successfully', 'success');
      }
      ...
      const errorMsg = error.response?.data?.message || 'Failed to save invoice structure';
      setErrorMessage(errorMsg);
      showToast(errorMsg, 'error');
    } finally {
      setIsProcessing(false);
